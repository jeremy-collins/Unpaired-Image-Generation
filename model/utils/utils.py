...
def visualize_data(
    ...
    if output is not None:
        if hasattr(config, "DIFFUSION") and config.DIFFUSION:
            ...
            if sample_diffusion:
                img_output = model.diffusion.sample(
                    model.unet, 1, model.combined_embedding[0], cfg_scale=0
                )[0]
            ...
            gt_img = cv2.putText(
                gt_img,
                "noise step: " + str(output["noise_step"][0].item()),
                (0, 75),
        ...
        if not mask_img and not mask_text:  # vae
            ...
            disp_pred_img = cv2.putText(
                img_output,
                "vae output: " + text_output,
                (0, 50),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (0, 0, 1),
            ...
            disp_pred_img = cv2.putText(
                zero_img,
                "pred text from image: " + text_output,
                (0, 50),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.5,
                (0, 0, 1),

...
def custom_collate_fn(batch):
    ...
    if text_input["input_ids"].size(1) < config.MAX_SEQ_LEN:
        text_input["input_ids"] = torch.nn.functional.pad(
            ...
            (0, config.MAX_SEQ_LEN - text_input["input_ids"].shape[1]),
        )
    else:
    ...
    text_input["attention_mask"] = (
        text_input["input_ids"] != model.tokenizer.pad_token_id
    )
...
with torch.no_grad():
    for i, (img, text) in enumerate(tqdm(loader)):
        ...
        print(
            "combined embedding means (mean, std): ",
            output["combined_embedding_means"].mean(),
            output["combined_embedding_means"].std(),
        )
        print(
            "combined embedding logvars (mean, std): ",
            output["combined_embedding_logvars"].mean(),
            output["combined_embedding_logvars"].std(),
        )

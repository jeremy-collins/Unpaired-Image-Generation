...
from model.models.pl_resnet_ae import *  # resnet encoder and decoder
import numpy as np
from model.utils.config_utils import parse_config_args
from model.models.t2idiffusion import Diffusion
...
class T2IVAE(nn.Module):
    ...
    def build_model(self):
        if (
            not hasattr(self.config, "ARCHITECTURE")
            or self.config.ARCHITECTURE == "resnet18"
        ):
            self.img_encoder = get_resnet18_encoder(
                first_conv=False, maxpool1=False
            ).to(self.device)
            self.gaussian_img_decoder = get_resnet18_decoder(
                latent_dim=self.config.LATENT_DIM,
                input_height=self.img_size,
                first_conv=False,
                maxpool1=False,
                nc=6,
            ).to(self.device)
        elif self.config.ARCHITECTURE == "resnet50":
            self.img_encoder = get_resnet50_encoder(
                first_conv=False, maxpool1=False
            ).to(self.device)
            self.gaussian_img_decoder = get_resnet50_decoder(
                latent_dim=self.config.LATENT_DIM,
                input_height=self.img_size,
                first_conv=False,
                maxpool1=False,
            ...
            self.img_encoder = get_resnet101_encoder(
                first_conv=False, maxpool1=False
            ).to(self.device)
            self.gaussian_img_decoder = get_resnet101_decoder(
                latent_dim=self.config.LATENT_DIM,
                input_height=self.img_size,
                first_conv=False,
                maxpool1=False,
            ...
            self.img_encoder = get_resnet152_encoder(
                first_conv=False, maxpool1=False
            ).to(self.device)
            self.gaussian_img_decoder = get_resnet152_decoder(
                latent_dim=self.config.LATENT_DIM,
                input_height=self.img_size,
                first_conv=False,
                maxpool1=False,
        ...
        t5_size = "small"
        self.t5 = T5ForConditionalGeneration.from_pretrained("t5-" + t5_size).to(
            self.device
        )
        ...
        if hasattr(self.config, "DIFFUSION") and self.config.DIFFUSION:
            ...
            self.unet = UNet_conditional(
                num_classes=self.config.LATENT_DIM, time_dim=self.config.LATENT_DIM
            ).to(self.device)
        else:
            self.diffusion = None
            self.unet = None
    ...
    def get_combined_embedding(self, img_feats, text_feats):
        if (
            not hasattr(self.config, "CONDITIONING")
            or self.config.CONDITIONING == "concat-mlp"
        ):
    ...
    def forward(
        self, img, text_inputs, mask_img=False, mask_text=False, use_diffusion=False
    ):
        text = text_inputs["input_ids"].to(
            self.device
        ...
        self.placeholder_input = torch.full(
            (text.shape[0], self.config.MAX_SEQ_LEN), self.tokenizer.pad_token_id
        ).to(
            self.device
        )  # to address end of batch error
        self.placeholder_input[
            :, 0
        ] = self.tokenizer.eos_token_id  # set first token to be start token

        # text encoder
        text_encoder_out = self.t5.encoder(
            input_ids=text, attention_mask=text_inputs["attention_mask"].to(self.device)
        )
        ...
        if self.args.sample_latent:
            ...
            self.combined_embedding = self.sample_gaussian(
        ...
        if use_diffusion:
            ...
            pred_img_logvars = pred_img_gaussian[
                :, 3:, :, :
            ]  # lowering logvars by subtracting a constant (e.g. 3-5ish)
        ...
        text_decoder_input = self.text_decoder_proj(self.combined_embedding).view(
            -1, self.config.MAX_SEQ_LEN, 512
        )
        text_decoder_out = self.t5.decoder(
            input_ids=self.placeholder_input,
            encoder_hidden_states=text_decoder_input,
            encoder_attention_mask=text_inputs["attention_mask"].to(self.device),
        )
        pred_text = self.t5.lm_head(
            text_decoder_out.last_hidden_state
...
if __name__ == "__main__":
    ...
    decoded_text = model.tokenizer.batch_decode(
        output["pred_text"], skip_special_tokens=True
    )
